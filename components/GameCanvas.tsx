...
const generateTextures = (): TexturePack => {
  ...
  // SPRITES
  const sprites: Record<number, HTMLCanvasElement> = generateSpriteTextures();
...
export const GameCanvas: React.FC<GameCanvasProps> = ({ onPlayerUpdate }) => {
  ...
  const render = (canvas: HTMLCanvasElement, p: PlayerState) => {
    ...
    for (const sprite of spriteData) {
      ...
      const spriteHeight = Math.abs(Math.floor((h / sprite.transformY) * scale));
      const spriteWidth = Math.abs(Math.floor((h / sprite.transformY) * scale));

      // Anchor sprite bottom slightly below floor boundary
      const spriteBottom = h / 2 + spriteHeight * 0.8;
      let drawStartY = spriteBottom - spriteHeight;
      if (drawStartY < 0) drawStartY = 0;
      let drawEndY = spriteBottom;
      if (drawEndY >= h) drawEndY = h - 1;

      // Apply per-sprite vertical offset (fraction of sprite height, negative lifts up)
      const yOffsetFactor = sprite.yOffset ?? 0;
      const yOffsetPx = Math.floor(yOffsetFactor * spriteHeight);
      drawStartY += yOffsetPx;
      drawEndY += yOffsetPx;
